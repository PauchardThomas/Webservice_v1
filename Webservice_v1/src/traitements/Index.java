...
public ArrayList<ZoneChalandise> listerZoneChalandisePourvues() throws Exception {
	...
	String select = "select id , intitule from zone_chalandise where etat = 1 ORDER BY intitule"; // Initialise la requète
	// System.out.println(select);
	 ...
	 try {
			...
			System.out.println("Prep1 : "+prep1);
			System.out.println("select : "+select);
			results = prep1.executeQuery(); // On execute la requète.
	 }catch(Exception e) {
		 e.printStackTrace(); // Sinon on affiche l'erreur
	 }
	 ...
	 while (results.next()) { // Tant qu'on trouve une réponse
		 ...
		 int Id = results.getInt("id"); // On récupère l'ID de la zone de chalandise renvoyée  par la BDD, dans une varaible.
		 String intitule = results.getString("intitule"); // On récupère la zone de chalandise renvoyée par la BDD, dans une variable.
		 ZoneChalandise zonechalandise = new ZoneChalandise(intitule, Id); // On créé notre objet nouvelle zone de chalandise avec un intitule et un ID
	 ...
	 return liste; // on renvoie la liste de zone(s) de chalandise pourvues.
...
public ArrayList<ZoneChalandise> listerZoneChalandiseNonPourvues() throws Exception {
	...
	String select = "select id , intitule from zone_chalandise where etat = 0 ORDER BY intitule"; // Initialise la requète
	 ...
	 while (results.next()) { // Tant qu'on trouve une réponse
		 ...
		 String intitule = results.getString("intitule"); // On récupère la zone de chalandise renvoyée par la BDD, dans une variable
		 ZoneChalandise zonechalandise = new ZoneChalandise(intitule, Id); // On créé notre objet nouvelle zone chalandise avec un intitule et un ID
	 ...
	 return liste; // on renvoie la liste de zone(s) de chalandise non pourvues.
